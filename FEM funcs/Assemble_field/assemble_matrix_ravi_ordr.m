function S = assemble_matrix_ravi_ordr(op1, f1, op2, f2, v, elements, msh, S,IP,basis)
% S = assemble_matrix returns the matrix struct S
% for higher order mesh 
...
%   
%   Possible input:
%   op1,op2 = '', 'grad', 'curl' or 'div;Note: only grad for higher order
%   f1,f2 = 'nodal', 'vector1', 'Nedelec', 'RT';Note: only nodal for higher
%   order
...
elseif strcmp(f1, 'nodal')
    ...
    elseif strcmp(op1, 'grad')
        F1 = @(ind_n, X)( bsxfun(@times, 1 ./ detF, ...
            mappingTimesVector(basis.gradPhi_ref(ind_n,2*X-1:2*X)', 1, 1, F) ) );
...
[X_quad, W_quad] = math_course_inttri(IP);
    ...
    for ind_1 = 1:NO_f1
        for ind_2 = 1:NO_f2
            E{ind_1, ind_2} = E{ind_1, ind_2} + W_quad(k_quad) * ...
                dotProduct( F1(ind_1, k_quad), F2(ind_2, k_quad) );
        end
    end
end
