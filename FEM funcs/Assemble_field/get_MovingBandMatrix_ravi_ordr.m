function S_ag = get_MovingBandMatrix_ravi_ordr(rotorAngle, IP,basis,varargin)
%get_MovingBandMatrix moving band stiffness matrix for higher order elements.
% 
...
if (numel(varargin) == 1) || ( (numel(varargin)==2) && isa(varargin{2}, 'double'))
    ...
    if ~isfield(msh, 'symmetrySectors')
       ...
       S_ag_struct = assemble_matrix_ravi_ordr('grad', 'nodal', 'grad', 'nodal', 1/mu0, [], struct('t', t_ag, 'p', p), [],IP,basis);
       %S_ag_struct = assemble_matrix_ravi2(1/mu0, struct('t', t_ag, 'p', p),[],basis);
